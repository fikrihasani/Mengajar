...
int main(){
	...
	for(int tc=1; tc<=T; tc++){
		...
		for(int i = 0; i<26; i++){
			printf("%d %d %d",freq[i],freqPercobaanMax[i],freqPercobaanMin[i]);
		}
		...
		alph[1] = 'a' + idxMin;
		printf("Case %d: ", tc);
		if(freq[idxMax] == freq[idxMin]) printf("N/A");							//artinya freq dari min ke max sudah rata, tidak perlu buang apapun
		else if(countDiffMax == 0 && countDiffMin == 0){
			if(alph[1] > alph[0]) printf("%c atau %c", alph[0], alph[1] );			//print sesuai urutan alphabet
			else printf("%c atau %c", alph[1], alph[0] );
		}
		else if(countDiffMax == 0 && countDiffMin == 1){						//frekuensi sama dengan buang max && tidak buang min
			printf("%c", alph[0]);
		}
		else if(countDiffMax == 1 && countDiffMin == 0){						//frekuensi sama dengan tidak buang max namun hanya buang min
			printf("%c", alph[1]);
		}
